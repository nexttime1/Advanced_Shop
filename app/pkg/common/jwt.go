...
func GetAuthUser(c *gin.Context) (int32, int, error) {
	...
	if !exists {
		return 0, 0, errors.WithCode(code.ErrUnauthorized, "未登录")
	}
	// 转换userid为uint  JWT解析后默认是float64
	userIdFloat, ok := userIdVal.(float64)
	if !ok {
		return 0, 0, errors.WithCode(code.ErrInvalidUserID, "用户ID格式错误")
	}
	...
	if !exists {
		return 0, 0, errors.WithCode(code.ErrRoleNotConfigured, "用户角色未配置")
	}
	// 转换role为int
	roleFloat, ok := roleVal.(float64)
	if !ok {
		return 0, 0, errors.WithCode(code.ErrInvalidRoleFormat, "用户角色格式错误")
	}
	role := int(roleFloat)

	// 校验角色合法性
	if role != 1 && role != 2 {
		return 0, 0, errors.WithCode(code.ErrInvalidRole, "角色不合法（仅支持1=管理员/2=普通用户）")
	}
